...
function prepareFieldMap(){
  ...
  const workflow=document.createElement("section");workflow.className="cadastre-workflow";workflow.innerHTML=`<div class="workflow-head"><strong>${lang==="de"?"Katastererfassung":"Cadastre capture"}</strong><span>Syrien → Damaskus</span></div><div class="workflow-steps"><div><b>1</b><span>${lang==="de"?"Flur verwalten":"Manage section"}</span><select id="workflow-section"><option value="">—</option></select><button id="workflow-new-section" type="button">${lang==="de"?"Neue Flur":"New section"}</button></div><div><b>2</b><span>${lang==="de"?"Flurstück erfassen":"Capture parcel"}</span><button id="workflow-capture-parcel" type="button">${lang==="de"?"Auf Karte zeichnen":"Draw on map"}</button></div><div><b>3</b><span>${lang==="de"?"Gebäude & Adresse":"Building & address"}</span><button id="workflow-assign-building" type="button">${lang==="de"?"Zuordnen":"Assign"}</button></div></div><p id="workflow-message"></p>`;
  ...
  form.insertAdjacentHTML("afterbegin",`<div class="address-form-heading"><span><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 20V8l8-5 8 5v12"></path><path d="M8 20v-7h8v7M3 20h18"></path></svg></span><strong>${lang==="ar"?"بيانات المبنى والعنوان":lang==="de"?"Gebäude- und Adressdaten":"Building and address data"}</strong></div>`);
  applyLanguage();
...
async function runDataFilter(){
  ...
  let items=[];
  if(type==="roads"){
    if(query.length<2){$("#filter-results").innerHTML=`<p class="filter-count">${lang==="de"?"Bitte mindestens zwei Zeichen eines Straßennamens eingeben.":"Enter at least two characters of a road name."}</p>`;return}
    const [response,data]=await api(`/api/v1/catalog/search?q=${encodeURIComponent(query)}`);
    if(!response.ok){$("#filter-results").innerHTML=`<p class="filter-count">${esc(data.error||"Search unavailable")}</p>`;return}
    items=(data.items||[]).filter(item=>item.object_type==="SYRIA_ROAD");
  ...
  $("#filter-results").innerHTML=`<p class="filter-count">${items.length} ${lang==="de"?"Treffer":"results"}</p>`+items.map(item=>item.object_type==="SYRIA_ROAD"?`<article class="request road-search-result"><div class="request-top"><div><strong dir="auto">${esc(item.label_ar||item.label_en||item.technical_code)}</strong>${item.label_en&&item.label_en!==item.label_ar?`<small>${esc(item.label_en)}</small>`:""}</div><b>${lang==="de"?"Straße":"Road"}</b></div><div class="meta">${esc(item.technical_code)} · OSM · ${esc(item.official_status||"")}</div><button class="ghost show-road-on-map" data-lon="${Number(item.longitude)}" data-lat="${Number(item.latitude)}" data-label="${esc(item.label_ar||item.label_en||item.technical_code)}">⌖ ${lang==="de"?"Auf Karte anzeigen":"Show on map"}</button></article>`:`<article class="request"><div class="request-top"><strong>${esc(item.name_ar||item.street_name_ar||item.official_code)} ${esc(item.house_number||"")}</strong><b>${esc(item.official_status||item.status||"")}</b></div><div class="meta">${esc(item.official_code||item.building_ref||"")} · ${esc(item.postal_code||"")} · ${item.floors||0} ${lang==="de"?"Etagen":"floors"} · ${item.dwelling_units||0} ${lang==="de"?"Wohnungen":"units"}</div></article>`).join("");
  document.querySelectorAll(".show-road-on-map").forEach(button=>button.onclick=()=>{openPortalPage("register");setTimeout(()=>{fieldMap?.flyTo({center:[+button.dataset.lon,+button.dataset.lat],zoom:17,duration:800});cadMessage(button.dataset.label)},80)});
}
