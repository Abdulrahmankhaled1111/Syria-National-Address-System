...
function buildPortalPages(){
  ...
  work.innerHTML=`<section class="app-launcher"><div class="app-launcher-head"><div><span class="eyebrow">${lang==="de"?"Nationales Kataster":"National register"}</span><h2>${lang==="de"?"Fachbereiche":"Specialist applications"}</h2><p>${lang==="de"?"Jeder Arbeitsbereich ist eine eigenständige App mit klar getrennten Daten und Aufgaben.":"Each workspace is a separate app with clearly separated data and tasks."}</p></div><div class="scope-command"><small>${lang==="de"?"Mein Zuständigkeitsbereich":"My scope"}</small><strong>${esc(organisation||"–")}</strong><span class="scope-secure">● ${lang==="de"?"Rollen- und gebietsgebunden":"Role and area restricted"}</span></div></div><div class="app-grid"><button data-open-app="register"><i>⌂</i><strong>${lang==="de"?"Kataster & Adressen":"Cadastre & addresses"}</strong><span>${lang==="de"?"Gebäude, Eingänge, Einheiten und Hausnummern":"Buildings, entrances, units and house numbers"}</span></button><button data-open-app="workflow"><i>✓</i><strong>${lang==="de"?"Vorgänge & Prüfung":"Cases & review"}</strong><span>${lang==="de"?"Anträge erfassen, prüfen und freigeben":"Create, review and approve requests"}</span></button><button data-open-app="field"><i>▦</i><strong>${lang==="de"?"Außendienst":"Field operations"}</strong><span>${lang==="de"?"Druck, Beschilderung und Montage":"Printing, signage and installation"}</span></button><button data-open-app="collaboration"><i>↔</i><strong>${lang==="de"?"Behördenaustausch":"Authority exchange"}</strong><span>${lang==="de"?"Übergaben, Fristen und Zusammenarbeit":"Handovers, deadlines and collaboration"}</span></button><button data-open-app="search"><i>⌕</i><strong>${lang==="de"?"Zentrale Suche":"Central search"}</strong><span>${lang==="de"?"Amtliche Objekte schnell finden":"Find official objects quickly"}</span></button></div></section>`;
  ...
  nav.innerHTML=`<div class="portal-nav-heading"><strong>${lang==="de"?"Nationales Kataster":"National register"}</strong><small>${lang==="de"?"Getrennte Fach-Apps":"Separate specialist apps"}</small></div><button data-page="work" class="active"><i><svg viewBox="0 0 24 24"><rect x="4" y="4" width="6" height="6" rx="1"></rect><rect x="14" y="4" width="6" height="6" rx="1"></rect><rect x="4" y="14" width="6" height="6" rx="1"></rect><rect x="14" y="14" width="6" height="6" rx="1"></rect></svg></i><span>${lang==="de"?"App-Übersicht":"App overview"}</span></button><button data-page="register"><i>⌂</i><span>${lang==="de"?"Kataster & Adressen":"Cadastre & addresses"}</span></button><button data-page="workflow"><i>✓</i><span>${lang==="de"?"Vorgänge & Prüfung":"Cases & review"}</span></button><button data-page="field"><i>◎</i><span>${lang==="de"?"Außendienst":"Field operations"}</span></button><button data-page="collaboration"><i>↔</i><span>${lang==="de"?"Behördenaustausch":"Authority exchange"}</span></button><button data-page="search"><i>⌕</i><span>${lang==="de"?"Suche":"Search"}</span></button><div class="portal-nav-footer"><span class="nav-status-dot"></span><small>${lang==="de"?"System verfügbar":"System available"}</small></div>`;
