...
$("#profile-tasks").onclick=()=>{$("#profile-menu").classList.add("hidden");openPortalPage("work")};
...
function prepareFieldMap(){
  ...
  changeGraphics.innerHTML=`<button id="change-graphics-toggle" class="change-graphics-toggle" type="button" aria-expanded="false"><span>⌃</span><strong>${lang==="ar"?"رسومات التغيير":lang==="de"?"Änderungsgrafiken Homs":"Homs change graphics"}</strong><small>${lang==="de"?"Kartenart oder Objekt auswählen":"Select map type or object"}</small></button><div class="change-graphics-body"><button id="change-graphics-prev" class="change-graphics-nav" type="button" aria-label="Zurück">‹</button><div id="change-graphics-track" class="change-graphics-track"></div><button id="change-graphics-next" class="change-graphics-nav" type="button" aria-label="Weiter">›</button></div>`;
  $(".field-map-pane").append(changeGraphics);
  $("#change-graphics-toggle strong").textContent=lang==="ar"?"اكتشف الخرائط؟":lang==="de"?"Karten entdecken?":"Discover maps?";
  $("#change-graphics-toggle small").textContent=lang==="ar"?"اختر طريقة عرض الخريطة":lang==="de"?"Kartenform auswählen":"Select map view";
  ...
  $("#governorate-show").onclick=async()=>{
    ...
    if(selected==="ALL"){
      ...
      $(".cad-title").textContent=lang==="ar"?"السجل العقاري الوطني السوري":lang==="de"?"Nationales Liegenschaftskataster Syrien":"Syrian National Property Register";
    ...
    $(".cad-title").textContent=lang==="ar"?`السجل العقاري · ${item.name_ar}`:lang==="de"?`Liegenschaftskataster · Gouvernement ${item.name_en}`:`Property register · ${item.name_en} Governorate`;
  ...
  fieldMap.on("load",()=>{
    ...
    fieldMap.addLayer({id:"field-cadastral-road-casing",type:"line",source:"field-roads",minzoom:10,layout:{visibility:"none"},paint:{"line-color":"#7a3825","line-width":["interpolate",["linear"],["zoom"],10,3.5,17,14],"line-opacity":.9}});
    fieldMap.addLayer({id:"field-cadastral-roads",type:"line",source:"field-roads",minzoom:10,layout:{visibility:"none"},paint:{"line-color":"#efb66b","line-width":["interpolate",["linear"],["zoom"],10,2,17,10.5],"line-opacity":.96}});
    ...
    fieldMap.addLayer({id:"field-cadastral-labels",type:"symbol",source:"field-roads",minzoom:13,layout:{visibility:"none","symbol-placement":"line","text-field":["coalesce",["get","name_ar"],["get","name_en"],""],"text-font":["Open Sans Regular"],"text-size":12},paint:{"text-color":"#20272a","text-halo-color":"#ffffff","text-halo-width":2}});
    ...
    fieldMap.addLayer({id:"field-active-outside-gray",type:"fill",source:"field-active-outside-mask",paint:{"fill-color":"#737976","fill-opacity":.68,"fill-antialias":true}});
  ...
  const setFieldMapMode=mode=>{
    ...
    visible("street-field",mode!=="satellite");visible("satellite",mode==="satellite");
    visible("field-syria-outside-white",false);visible("field-active-outside-gray",true);visible("field-active-boundary-casing",true);visible("field-active-boundary",true);visible("field-3d-governorate-boundaries",!selectedGovernorateBoundary);visible("field-3d-selected-boundary",false);
    visible("field-buildings-3d",mode==="3d");visible("field-buildings-flat",mode==="cadastral");
    ["field-cadastral-sections","field-cadastral-section-lines","field-cadastral-section-labels"].forEach(id=>visible(id,true));
    ...
    if(fieldMap.getLayer("field-cadastral-roads")){fieldMap.setPaintProperty("field-cadastral-roads","line-color",mode==="roads"?"#c45136":"#efb66b");fieldMap.setPaintProperty("field-cadastral-roads","line-opacity",mode==="roads"?1:.96)}
  ...
  refreshChangeGraphics=()=>{
    ...
    const views=lang==="ar"?[["normal","الخريطة العامة","الطرق والمدن والحدود","overview"],["cadastral","خريطة السجل العقاري","العقارات والمباني والأرقام","cadastre"],["satellite","خريطة الأقمار الصناعية","صور جوية وحدود التغييرات","imagery"],["roads","خريطة الشوارع","الشوارع المسجلة وأسماؤها","roads"]]:lang==="de"?[["normal","Übersichtskarte","Straßen, Orte und Grenzen","overview"],["cadastral","Liegenschaftskarte","Flurstücke, Gebäude und Nummern","cadastre"],["satellite","Satellitenkarte","Luftbild und Änderungsgrenzen","imagery"],["roads","Straßenkarte","Erfasste Straßen und Straßennamen","roads"]]:[["normal","Overview map","Roads, places and boundaries","overview"],["cadastral","Property map","Parcels, buildings and numbers","cadastre"],["satellite","Satellite map","Imagery and change boundaries","imagery"],["roads","Street map","Captured roads and street names","roads"]];
  ...
  $("#change-graphics-toggle").onclick=()=>{const drawer=$("#change-graphics-drawer"),collapsed=drawer.classList.toggle("collapsed");$("#change-graphics-toggle").setAttribute("aria-expanded",String(!collapsed));$("#change-graphics-toggle span").textContent=collapsed?"⌃":"⌄"};
  ...
  const legend=()=>cadMessage(lang==="de"?"Legende: Orange = Entwurf · Schwarz = freigegeben · Grau = Gebäude · Zahlen = Flur/Flurstück und Hausnummer":"Legend: orange draft · black approved · grey building.");
...
async function loadBuildings(){
  ...
  let [d,manual,roads,numbers,sections,parcels,boundary]=await Promise.all([
  ...
  fieldBuildings=d;fieldRoads=roads;fieldNumbers=numbers;fieldSections=sections;fieldParcels=parcels;fieldBoundary=boundary;
...
function buildPortalPages(){
  const dashboard=$("#dashboard"),children=[...dashboard.children],work=document.createElement("div");
  work.id="page-work";work.className="portal-page";
  children.forEach(child=>work.appendChild(child));
  work.insertAdjacentHTML("afterbegin",`<section class="inventory-command"><div class="inventory-command-head"><div><span class="eyebrow">${lang==="ar"?"التعاون بين الجهات":lang==="de"?"Behördenübergreifende Zusammenarbeit":"Inter-authority collaboration"}</span><h2>${lang==="ar"?"مركز عمل بيانات السجل":lang==="de"?"Arbeitszentrale Bestandsdaten":"Register data work centre"}</h2><p>${lang==="ar"?"المهام والتحويلات والمواعيد وجودة البيانات ضمن نطاق المسؤولية.":lang==="de"?"Aufgaben, Übergaben, Fristen und Datenqualität im eigenen Zuständigkeitsbereich.":"Tasks, handovers, deadlines and data quality within the assigned scope."}</p></div><div class="scope-command"><small>${lang==="ar"?"نطاقي":lang==="de"?"Mein Zuständigkeitsbereich":"My scope"}</small><strong id="collaboration-scope">${esc(organisation||"–")}</strong><span class="scope-secure">● ${lang==="ar"?"مقيد بالصلاحيات":lang==="de"?"Rollen- und gebietsgebunden":"Role and area restricted"}</span></div></div><div class="collaboration-kpis"><article><span>↔</span><div><strong id="collab-open">0</strong><small>${lang==="ar"?"معاملات مفتوحة":lang==="de"?"Offene Übergaben":"Open handovers"}</small></div></article><article><span>!</span><div><strong id="collab-due">0</strong><small>${lang==="ar"?"مواعيد مستحقة":lang==="de"?"Fällige Fristen":"Due deadlines"}</small></div></article><article><span>▲</span><div><strong id="collab-urgent">0</strong><small>${lang==="ar"?"أولوية عالية":lang==="de"?"Hohe Priorität":"High priority"}</small></div></article><article><span>✓</span><div><strong id="collab-completed">0</strong><small>${lang==="ar"?"مكتمل":lang==="de"?"Abgeschlossen":"Completed"}</small></div></article></div><div class="collaboration-grid"><div class="collaboration-inbox"><div class="section-title"><h3>${lang==="ar"?"صندوق المهام المشترك":lang==="de"?"Gemeinsamer Behörden-Posteingang":"Shared authority inbox"}</h3><button id="refresh-collaboration" class="small ghost">↻</button></div><div id="collaboration-cases" class="collaboration-cases"></div></div><form id="collaboration-form" class="collaboration-form"><div class="section-title"><h3>${lang==="ar"?"تحويل جديد":lang==="de"?"Neue Behördenübergabe":"New authority handover"}</h3><span>↗</span></div><label>${lang==="ar"?"الموضوع":lang==="de"?"Betreff":"Subject"}<input id="collab-title" maxlength="180" required></label><div class="collaboration-form-row"><label>${lang==="ar"?"نوع المعاملة":lang==="de"?"Vorgangsart":"Case type"}<select id="collab-type"><option value="BUILDING_PERMIT">Building permit</option><option value="BOUNDARY_REVIEW">Boundary review</option><option value="ADDRESS_ASSIGNMENT">Address assignment</option><option value="DATA_CORRECTION">Data correction</option><option value="INTER_AUTHORITY_REVIEW">Inter-authority review</option><option value="COORDINATION">Coordination</option></select></label><label>${lang==="ar"?"الأولوية":lang==="de"?"Priorität":"Priority"}<select id="collab-priority"><option value="NORMAL">Normal</option><option value="HIGH">High</option><option value="CRITICAL">Critical</option><option value="LOW">Low</option></select></label></div><label>${lang==="ar"?"الجهة المستلمة":lang==="de"?"Empfangende Behörde":"Receiving authority"}<select id="collab-target" required></select></label><div class="collaboration-form-row"><label>${lang==="ar"?"الدور المسؤول":lang==="de"?"Zielrolle":"Target role"}<select id="collab-role"><option value="MUNICIPAL_EDITOR">Building office</option><option value="SURVEYOR">Survey</option><option value="REVIEWER">Review</option><option value="APPROVER">Approval</option><option value="REGISTRY_OFFICER">Registry</option></select></label><label>${lang==="ar"?"الموعد":lang==="de"?"Frist":"Deadline"}<input id="collab-due-at" type="date"></label></div><label>${lang==="ar"?"الوصف":lang==="de"?"Arbeitsauftrag":"Work instruction"}<textarea id="collab-description" maxlength="1200"></textarea></label><button type="submit">${lang==="ar"?"إرسال بشكل موثق":lang==="de"?"Nachvollziehbar übergeben":"Submit auditable handover"}</button><p id="collab-message" class="muted"></p></form></div></section><div class="inventory-section-divider"><strong>${lang==="ar"?"بيانات السجل والتحديث":lang==="de"?"Fachdaten und Katasterfortführung":"Domain data and cadastral maintenance"}</strong><span>${lang==="ar"?"تعديل فقط عبر مسار المراجعة":lang==="de"?"Änderungen ausschließlich über geprüfte Vorgänge":"Changes only through reviewed cases"}</span></div>`);
  const nav=document.createElement("nav");nav.className="portal-nav";
  nav.innerHTML=`<div class="portal-nav-heading"><strong>${lang==="ar"?"السجل الوطني":lang==="de"?"Nationales Kataster":"National register"}</strong><small>${lang==="ar"?"العقارات والعناوين":lang==="de"?"Liegenschaften & Adressen":"Property & addresses"}</small></div><button data-page="work" class="active"><i><svg viewBox="0 0 24 24"><rect x="4" y="4" width="16" height="16" rx="2"></rect><path d="M8 8h3v3H8zM13 8h3M13 11h3M8 14h3v3H8zM13 14h3M13 17h3"></path></svg></i><span>${lang==="ar"?"بيانات السجل":lang==="de"?"Bestandsdaten":"Register data"}</span></button><button data-page="search"><i><svg viewBox="0 0 24 24"><circle cx="10.5" cy="10.5" r="5.5"></circle><path d="m15 15 5 5"></path></svg></i><span>${lang==="ar"?"البحث":lang==="de"?"Suche":"Search"}</span></button><div class="portal-nav-footer"><span class="nav-status-dot"></span><small>${lang==="ar"?"النظام متاح":lang==="de"?"System verfügbar":"System available"}</small></div>`;
  dashboard.append(nav,work);
...
function openPortalPage(name){
  ...
  if(name==="settings"){loadSystemSettings();loadGoogleExportStatus()}if(name==="support")loadSupportTickets();if(name==="search")runDataFilter();
...
async function loadHeaderNotifications(){
  ...
  document.querySelectorAll(".notification-item").forEach(button=>button.onclick=()=>{$("#notification-menu").classList.add("hidden");$("#notification-toggle").setAttribute("aria-expanded","false");openPortalPage("work")});
}
...
initHeaderNotifications();const baseShow=show;show=function(){baseShow();syncHeaderNotifications()};
