...
function show(){if(token){$("#login").classList.add("hidden");$("#dashboard").classList.remove("hidden");$("#header-profile").classList.remove("hidden");document.body.dataset.userRole=role;const roleName=role==="SYSTEM_ADMIN"?(lang==="ar"?"مدير النظام":lang==="de"?"Systemadministrator":"System administrator"):(organisation||"Stadtverwaltung");$("#identity").textContent=roleName+" · "+(lang==="ar"?"مسجل":lang==="de"?"angemeldet":"signed in");$("#profile-name").textContent=roleName;$("#profile-scope").textContent=role==="SYSTEM_ADMIN"?(lang==="ar"?"الوصول الوطني":lang==="de"?"Nationaler Zugriff":"National access"):(organisation||"");$("#profile-initial").textContent=role==="SYSTEM_ADMIN"?"A":"G";load()}else{$("#login").classList.remove("hidden");$("#dashboard").classList.add("hidden");$("#header-profile").classList.add("hidden");$("#profile-menu").classList.add("hidden");delete document.body.dataset.userRole;$("#identity").textContent=messages[lang].signedOut}}
...
function prepareFieldMap(){
  ...
  $("#parcel-search-submit").onclick=async()=>{
    ...
    cadMessage(`${lang==="de"?"Ausgewählt":"Selected"}: Flur ${feature.properties.section_number} · Flurstück ${feature.properties.parcel_number}`);
  };
  ...
  $("#workflow-edit-parcel").onclick=()=>{const id=$("#house-parcel")?.value,feature=fieldParcels?.features?.find(item=>String(item.id)===String(id));if(!feature)return;startCapture();capturePoints=feature.geometry.coordinates[0].slice(0,-1).map(point=>[...point]);$("#parcel-section").value=feature.properties.section_number;$("#parcel-number").value=feature.properties.parcel_number;$("#parcel-quality").value=feature.properties.quality_level||"D";updateCapture();$("#workflow-message").textContent=lang==="de"?"Grenzpunkte verschieben oder neu setzen und anschließend erneut einreichen.":"Edit boundary points and submit again."};
  ...
  $("#parcel-save").onclick=async()=>{
    ...
    if(captureTarget==="section"){
      ...
      $("#workflow-message").textContent=lang==="de"?`Flur ${result.section_number} ist als abgegrenzte Fläche gespeichert. Jetzt können darin Flurstücke erfasst werden.`:`Section ${result.section_number} is stored as a bounded area. Parcels can now be captured inside it.`;
      return
    }
    if(captureTarget==="building"){
      ...
      $("#workflow-message").textContent=lang==="de"?`Gebäudeobjekt ${result.object_number} ist als Entwurf gespeichert. Haustürpunkt setzen, Straße und Hausnummer eintragen und einreichen.`:`Building object ${result.object_number} saved as draft. Set the entrance point and assign the address.`;
      return
    }
    const [response,result]=await api("/api/v1/cadastre/zabadani/parcels/capture","POST",{admin_unit_id:activeAdminUnitId,section_number:section,parcel_number:parcel,
      quality_level:$("#parcel-quality").value,geometry:{type:"Polygon",coordinates:[[...capturePoints,capturePoints[0]]]},
    ...
    $("#parcel-capture-status").textContent=lang==="de"?`✓ Flurstück ${section}/${parcel} wurde ${result.updated?"korrigiert":"neu angelegt"} und dauerhaft gespeichert. Status: ENTWURF.`:`✓ Parcel ${section}/${parcel} ${result.updated?"updated":"created"} and permanently stored. Status: DRAFT.`;
...
function buildPortalPages(){
  ...
  nav.innerHTML=`<div class="portal-nav-heading"><strong>${lang==="ar"?"السجل الوطني":lang==="de"?"Nationales Kataster":"National register"}</strong><small>${lang==="ar"?"العقارات والعناوين":lang==="de"?"Liegenschaften & Adressen":"Property & addresses"}</small></div><button data-page="work" class="active"><i>▦</i><span>${lang==="ar"?"بيانات السجل":lang==="de"?"Bestandsdaten":"Register data"}</span></button><button data-page="search"><i>⌕</i><span>${lang==="ar"?"البحث":lang==="de"?"Suche":"Search"}</span></button><button data-page="settings"><i>⚙</i><span>${lang==="ar"?"النظام":lang==="de"?"System":"System"}</span></button><button data-page="support"><i>?</i><span>${lang==="ar"?"الدعم":lang==="de"?"Support":"Support"}</span></button><div class="portal-nav-footer"><span class="nav-status-dot"></span><small>${lang==="ar"?"النظام متاح":lang==="de"?"System verfügbar":"System available"}</small></div>`;
